...
test "part 1" {
    ...
    const hands = @embedFile("hands.txt");
    var lines = std.mem.splitScalar(u8, hands, '\n');
    ...
    while (lines.next()) |line| {
        if (line.len > 0) {
            try players.append(try Player.fromLine(line, false));
            index += 1;
        }
    }
...
test "part 2" {
    ...
    while (lines.next()) |line| {
        if (line.len > 0) {
            try players.append(try Player.fromLine(line, true));
            index += 1;
        }
    }
...
const Player = struct {
    ...
    fn fromLine(line: []const u8, joker: bool) !Player {
